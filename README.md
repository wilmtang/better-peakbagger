...
`Start` = first chronological point, `Summit` = timestamp of the highest *adjusted* elevation, `Back to car` = last point; `Time to summit` / `Time back` follow. All clock times and day boundaries use the **mountain's local time**, not the viewer's: the summit coordinate is resolved to an IANA timezone by the bundled offline [tz-lookup](https://github.com/photostructure/tz-lookup) raster (no network, no coordinates leave the page), and `Intl` then applies the correct political offset and DST for the trip's date. The stats bar names the zone, e.g. *"Times in the mountain's local time (PDT)"*. If the lookup ever fails, times fall back to solar time rounded to the whole hour from the summit longitude and the label says so (*"UTC−8, estimated from longitude"*). A *relative-day* helper converts each timestamp to mountain-local midnight and diffs against the start date; if the trip spans >1 calendar day it prefixes tooltips/axes/stats with `Day N`. **Camping** detection is purely chronological: whenever a point lands on a later calendar day than its predecessor, the *predecessor's* coordinates are the camp for that night. Being chronological (not spatial), it's immune to overnight GPS drift.
