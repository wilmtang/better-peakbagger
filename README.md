...
Selected matches then open as inactive tabs in the **Peak Drafts** group. Each blank tab is assigned a private `{ jobId, tabId, pid, cid }` identity before navigation. On the ascent editor, `src/ascent-draft.js` sends a ready handshake; the background checks the sender tab plus `pid` and `cid` before returning any payload. Drafts submit GPS Preview one at a time in confidence-ranked tab order so Peakbagger never receives concurrent uploads from one capture. The content script verifies the expected form and privacy-reduced GPX, fills both metric and imperial fields plus the assigned suffix and enabled trip fields, attaches the file, records a Preview-start acknowledgement, and clicks `GPXPreview` exactly once. Encounter time remains analysis metadata and is never written into Peakbagger's suffix field.
