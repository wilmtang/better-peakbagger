...
// src/settings.js on the way into storage, and the page-world GPX analyzer,
...
test('no surface keeps its own copy of a schema default or bound', async () => {
    ...
    const sources = (await readdir(path.join(root, 'src')))
        .filter(name => name.endsWith('.js') && name !== 'settings-schema.js');
    ...
    for (const name of sources) {
        const text = await readFile(path.join(root, 'src', name), 'utf8');
        for (const { pattern, what } of banned) {
            if (pattern.test(text)) leaks.push(`src/${name} hardcodes ${what}`);
        }
    }
    assert.deepEqual(leaks, [],
        `these belong in src/settings-schema.js:\n${leaks.join('\n')}`);
});
