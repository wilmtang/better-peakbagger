...
- Page features stay separated by surface: `src/gpx-analyzer.js` owns ascent GPX analysis and map synchronization, `src/ascent-filter.js` owns PeakAscents filtering and in-DOM sorting, and `src/ascent-draft.js` owns validated draft filling. Prefer extending the owning surface over adding cross-feature globals.
