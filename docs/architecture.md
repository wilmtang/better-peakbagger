...
| `src/profile-backup-core.js` | Pure shared module | Signed-in owner discovery, numeric URL identity, and fail-closed Peakbagger response classification | Favorites persistence |
...
Peakbagger exposes no Buddy API used by this feature. Every network refresh reads
the authenticated HTML report at:

```text
/report/report.aspx?r=b&cid=<signed-in-owner-cid>
```

All fetches use `credentials: 'include'`. Before parsing, callers pass status,
headers, and body through `classifyResponse(..., { kind: 'buddies' })`. A
Cloudflare marker, 403/429/503 response, other failed status, login page, or body
without both the Buddy List marker and `RGridView` is rejected. The parser then
...
2. **Options actions.** The extension page cannot trust a page DOM for current
   identity, so it first fetches `https://www.peakbagger.com/Default.aspx` and
   extracts the signed-in owner. **Refresh now** forces the subsequent Buddy
   request even for a fresh cache. Merge and mirror perform the owner check but
   reuse a fresh cache only when its `ownerCid` matches; otherwise they refresh
   before changing the custom list.
...
| --- | --- | --- |
| Fresh, matching owner | Uses cached ids immediately | Automatic path skips the request; **Refresh now** still forces it |
...
| Owner cannot be detected | A previously stored cache remains usable, but an absent cache cannot load | No automatic page request; options reports sign-in state after its owner probe |
