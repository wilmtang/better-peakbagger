...
4. **Confirmed native Buddy mutation.** On another climber's public profile,
   ...
   `sessionStorage`; the next supported climber-page document consumes and
   deletes it, and accepts it only for the same target within five minutes.
   After Peakbagger completes the navigation, that document fetches the signed-in
   Buddy report. The response owner must match the current signed-in identity
   and the target's membership must match the intended action before the custom
   list changes. The validated report refreshes the Buddy cache even when an
   action is unconfirmed; request, identity, parse, or storage failure preserves
   both previously stored datasets.
...
- `test/options/options.test.mjs` covers source switching, identity-checked add, live
  totals and fuzzy search, reversible delete/mirror/restore, Buddy refresh,
  merge, the default-off removal preference, distance-aware navigation, and
