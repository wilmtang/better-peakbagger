...
4. **Frame render**: replies are re-validated all-or-nothing (like the route)
   ...
   built from the validated integer id only. The button owns pointer, click,
   and keyboard activation; MapLibre's layer-scoped events are never used.
   Refreshing the dots closes an open popup, matching the native marker rebuild.
...
The normal MapLibre 6 path renders each ring as a fixed 17px button through
`Marker`. MapLibre positions that DOM element with its terrain-aware transform,
while the button supplies an exact pointer and focus target. The retained
circle-layer fallback uses the frame's bounded `peakFeatureAt()` screen-space
scan only if a reduced MapLibre-compatible API has no `Marker` constructor.
...
The DOM ring and fallback circle both derive size and color from
`PEAK_MARKERS`. The normal marker remains a constant-size billboard like the
native 16×16 gif; the fallback sets `circle-pitch-scale: 'viewport'` and derives
its hit radius from the same `radius + strokeWidth` plus `hitSlopPx`.
...
keep those two representations and the fallback hit radius derived from that
single spec; requesting, validation, and popup wiring remain unaffected.
...
0. **A dot's height is never computed here — and its position is snapped to
   the rendered summit.** The feed's features carry only lat/lon; MapLibre's
   Marker transform projects each ring against the same Mapterhorn terrain
   the mountains use, with exaggeration 1. But three
...
1. **A dot can still slide on screen during a tilt or zoom — its height is
   ...
   elevation*: MapLibre reprojects the Marker from whatever DEM tiles are
...
3. **No terrain occlusion.** Summit dots are navigational overlays like the
   native feed, so the Marker uses the same opacity when MapLibre classifies
   its anchor as covered. This also avoids losing a summit ring when a coarse
   terrain depth sample and its geographic anchor are nearly coplanar.
...
6. The billboarded peak-name **labels** PoC (vendored glyphs, symbol layer)
   ...
   use DOM markers (with a circle fallback) and need no glyphs.
...
- Hidden real-browser check (`scripts/verify-terrain-visual.mjs`, headless
  ...
  composited page output by pixel color, click on the found ring opens the
